...
class NetworkManagerDriver(NetDriver):
    ...
    async def status(self) -> LinkStatus:
        ...
        if state == 100:
            addresses = await self._property(self.device_path, NM_DEVICE, "Ip4Config")
            ip = None if addresses == "/" else "connected"
            self._status = LinkStatus(phase="online", ssid=self._status.ssid, ip=ip)
        elif state in {30, 40, 50, 60, 70}:
            self._status = LinkStatus(phase="associating", ssid=self._status.ssid)
        elif state == 120:
            self._status = LinkStatus(
                phase="failed", ssid=self._status.ssid, err="WIFI_AUTH_FAILED"
            )
        return self._status
